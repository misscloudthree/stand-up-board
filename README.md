...
- **每日測試追蹤**：多人可各自輸入 FSD 模組的每日開單量／複測數量／複測通過量／Pre-UAT 處理量，並自動加總成當日 Summary。
...
- **每日測試追蹤**：先在「模組管理」新增 FSD 模組與負責人；選擇日期後，每個模組一列可輸入當日數字，按「儲存」寫回 GitHub；下方 Summary 會自動加總目前畫面上所有模組的數字。切換日期或有其他人更新資料後，按「重新整理」重新拉取最新資料（不是即時同步）。
