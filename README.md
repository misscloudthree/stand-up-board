...
- **Backlog 追蹤**：點「＋ 新增 Backlog」建立議題；點卡片可修改類型/負責人/deadline/狀態，並在下方留言記錄討論或處理進度；deadline 在兩天內或已逾期的卡片會自動標色提醒。工具列可勾選要顯示哪幾種狀態（預設只勾待處理/進行中）、依類型篩選、切換依 Deadline 近到遠或遠到近排序。
